...
Let's mark the function which maps a natural number $n$ to its corresponding $C_n$ HOF, as $C_E$ (E stands for encoding). The inverted function, which maps a result of $C_E$ to its original natural number, and will be named $C_D$ (D stands for decoding). We will name a HOF that was returned by $C_E$ a Church function, it's not a formal term, but it will make my sentences much shorter. 🙃
...
The type we defined above, `CNumber` is a container for Church function. Please notice that it doesn’t mean every value of type `CNumber` is actually a Church function, the container will match every HOF that receive a function and a value and returns a value.  
...
Assuming that `succ` receives a `CNumber` represents $n$. We define the result as a new function, which takes a function `f` and a value `x`. it is going to use `a` (the function that the given `CNumber` holds) to apply `f` on `x` $n$ times, and then apply `f` on the result once again. And so, we got the `CNumber` that represents $n+1$!  
...
But we can do better! We have the function `a`, which knows to apply a certain function $n$ times. Let's use it smarter. We can apply `succ` on `Nr b` (the `CNumber` that holds a Church function that represents $m$) $n$ times, and so, we will get the `CNumber` that holds a Church function that represents $n+m$.
