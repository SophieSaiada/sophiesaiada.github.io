...
פונקציה מסדר גבוה היא פונקציה שמקבלת פונקציה כלשהי כקלט, או מחזירה פונקציה כקלט. למעשה, אלה פונקציות שפועלות על פונקציות.  
...
כמובן שלא משתמשים באופן היצוג הזה בפרקטיקה, כי הרבה פשוט וזול לייצג בזכרון מספר ביצוג בינארי מאשר בתור פונקציה.  
...
לפני שנממש את פונקציית החיבור, נממש אחת פשוטה יותר בשם `succ`. מה שהיא עושה זה לקבל עצם מסוג `CNumber`, ולהחזיר את העצם הבא אחריו ביצוג. כלומר, אם היא קיבלה את `one` היא תחזיר את `two`. הפונקציה הזו מקבילה לפונקצית העוקב המספרים הטבעיים.
...
עכשיו יהיה לנו הרבה יותר קל להגדיר את פונקציית החיבור. אנחנו רוצים פונקציה בשם `plus` שתקבל שני עצמים מסוג `CNumber` ותחזיר את ה-`CNumber` שמתאר את החיבור של העצמים, לפי החיבור שהגדרנו למעלה.
